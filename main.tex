...
% Schema per la tecnologia:
% 1. Breve intro: cosa è, quando è stato introdotto, per quale mercato e chi lo usa
% 2. Come funziona
% 3. Come implementa l'integrità
% 4. Qualche attacco in letteratura
% 5. [EXTRA] se è stato fixato, o altra roba

\section{ARM TrustZone}
\label{sec:arm-trustzone}
ARM TrustZone è una etichetta introdotta da ARM nel 2004\cite{trustzone}
che viene usata per indicare un insieme di funzionalità hardware per la
...
Samsung Knox \cite{knox}, Trustonic TEE\cite{trustonic} e
OP-TEE\cite{optee}.
...
Le modalità di privilegio di ARM, che vanno da EL3(più alto) a EL0(più basso),
vengono duplicate nelle controparti S-EL3 a S-EL0.
...
Il sistema si avvia in modalità \emph{Secure} con il più alto livello di
privilegio supportato;
il firmware in questo stato può configurare il sistema per decidere quanta
memoria e quali periferiche isolare nel mondo \emph{Secure} e
\emph{Non-Secure}.

Una volta configurato il sistema, il firmware abbassa il livello
di privilegio e carica il sistema operativo del TEE e il sistema operativo
del REE.
Un esempio di firmware è \textit{Trusted Firmware-A}\cite{trusted-firmware-a},
una libera implementazione di riferimento sviluppata da ARM.

Per passare da una modalità all'altra esiste una istruzione speciale
\texttt{smc}(\textit{Secure Monitor Call}) che opera in modo simile alle
istruzioni \texttt{svc} e \texttt{hvc} per il passaggio da EL0 a EL1 e EL2.

\bigbreak \noindent

% FIXME: Come implementa il secure boot in TrustZone

\bigbreak \noindent

% FIXME: Breve overview degli attacchi a TrustZone

\section{Intel SGX}
\label{sec:intel-sgx}
Intel Software Guard Extensions (SGX) è una estensione
dell'architettura x86\_64 introdotta nel 2015\cite{sgx} con i processori
...
per questo motivo SGX è una delle poche soluzioni hardware oggi disponibili
per utilizzare un TEE su tali piattaforme.

\bigbreak \noindent

Nonostante questo, Intel SGX ha subito diversi attacchi mirati a minare
la proprietà di confidenzialità; molti di questi sono attacchi di tipo
\textit{side-channel} che sfruttano le proprietà fisiche del processore 
% FIXME: Parlare delle vulnerabilità di SGX
...
x86\_64 introdotta nel 2016\cite{sev} con la prima serie
di processori Ryzen.
...
Dopo la prima versione di SEV, AMD ha anche rilasciato
SEV-ES (\textit{Secure Encrypted Virtualization-Encrypted State}), per
crittografare anche lo stato dei registri, e
SEV-SNP (\textit{Secure Encrypted Virtualization-Secure Nested Paging}), 
che integra correzioni a vulnerabilità scoperte dopo il rilascio di SEV-ES.

Nonostante ciò, anche AMD SEV-SNP è stato dimostrato vulnerabile
ad altrettanti attacchi. % FIXME: Cita un po' di roba qui
...
\textit{Secure Enclave}\cite{secure_enclave}.
...
\textit{Titan M}.
L'attacco è una vulnerabilità software in una procedura di copia
che permette di scrivere un singolo byte a un indirizzo qualsiasi in memoria.
Questa vulnerabilità permette un attaccante di effettuare un attacco
...
Alcuni esempi sono Keystone\cite{keystone} e Penglai\cite{penglai}.
