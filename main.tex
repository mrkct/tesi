...
\section{Intel SGX}
\label{sec:intel-sgx}
% FIXME: Cita in qualche modo \cite{sgxexplained}
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|}
        \hline
        % FIXME: Scrivere i vincoli sulle dimensioni dei payload
...
% Abbiamo usato una VM dentro una VM per testare il nostro prototipo
% La VM esterna è Linux versione X con busybox X girata su QEMU X nella macchina virtuale qemu-virt-8
% La VM interna è Linux versione Y con buusybox Y girata sulla nostra versione modifcata di QEMU, cioè Y commit XXX
% Per verifcare la correttezza della nostra soluzione abbiamo usato tre set di test cases

% Il primo set sono i programmi di esempio forniti da OPTEE, questi includono
% sono: AES, SECURE STORAGE, ...
% Sono programmi relativemente semplici ma che testano tutte le funzionalità
% dell'interfaccia GlobalPlatform e che fanno uso di libteec

% Il secondo set è un sottoinsieme dei test di "xtest", una suite di test utilizzata
% per testare OPTEE.
% Il sottoinsieme scelto esclude le funzionalità specifiche di OPTEE che non sono incluse
% nelle specifiche GP, in totale sono 700 test cases

% Abbiamo inoltre effettuato dei semplici test delle performance, nonostante ribadiamo
% che questa implementazione sia intenzionalmente primitiva e inefficiente.
% Per misurare le performance abbiamo usato il comando time,
% che misura il tempo di esecuzione di un comando, utilizzando la misura 'real time'
% Ogni test case è stato eseguito 10 volte e i risultati riportanti in \tablename~\ref{tab:performance}
% A confronto abbiamo anche riportato i tempi di esecuzione degli stessi test case
% sulla VM host (cioè la VM esterna), che accede direttamente al TEE senza passare
% dal nostro passthrough.
% [TABELLA]
% Chiaramente i tempi di esecuzione utilizzando il passthrough sono più alti,
% principalmente per l'implementazione inefficiente della sincronizzazione dei buffer
% di memoria condivisa.
% Infatti, se dividiamo i test case in due categorie, quelli che fanno uso di memoria
% condivisa e quelli che non lo fanno, si nota che i tempi di esecuzione sono
% dei secondi sono molto simili ai tempi senza passthrough.
% [TABELLA]
% Chiaramente l'uso di test case per confrontare le performance non
% necessariamente riflette dei carichi di lavoro reali, ma è un modo semplice
% per avere un'idea delle prestazioni.
