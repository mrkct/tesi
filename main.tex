...
% - I TEE non possono essere considerati tutti uguali perchè implementati in maniera diversa
% - Nonostante questo GP definisce un set di proprietà minime che devono essere garantite da tutti i TEE
% - Queste proprietà sono elencate nel PP
% - I TEE possono garantire anche proprietà aggiuntive, ma non sono obbligatoriamente garantite da tutti i TEE
% - Sono 3 proprietà importanti: Isolation, Confidentiality, Integrity
...
% FIXME: Sarebbe interessante una sotto-sezione "Cosa non garantiscono i TEE"(side-channel, attacchi di tipo hardware, ecc)
...
I Trusted Execution Environment diventano più utilizzati di anno in anno. % FIXME: Cit required
Oggi i TEE sono largamente diffusi in ambito mobile, seppur non sempre
pubblicizzati, ma il loro utilizzo spazia in diversi settori.
...
l'installazione di applicazioni aggiuntive dentro il TEE dopo la
configurazione iniziale.

Questo potrebbe cambiare nei prossimi anni a seguito dell'adozione da
più vendor degli standard TEE GlobalPlatform e lo sviluppo di TEE open-source
come OP-TEE; di conseguenza potremmo vedere nei prossimi anni un uso più
ampio di questa tecnologia.

% FIXME: Forse una breve discussione sulla Remote Attestation, perchè è quella che abilita molte applicazioni
...
\textit{Platform Security Processor}\cite{amd2020ftpm}, utilizzano la
funzionalità di \textit{Secure Storage} per implementare un
\textit{Trusted Platform Module}(TPM) software.
